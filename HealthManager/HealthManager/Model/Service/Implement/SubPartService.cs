...
namespace HealthManager.Model.Service.Implement
{
    public class SubPartService : ISubPartService
    {
        ...
        /// <returns></returns>
        public bool RegistSubPart(int parentPartId, string subPartName)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public List<SubPartModel> GetSubPartDataList()
        {
            using (var db = new SQLiteConnection(DbConst.DbPath))
            {
        ...
        /// <param name="parentPartId"></param>
        /// <returns></returns>
        public List<SubPartModel> GetSubPartDataList(int parentPartId)
        {
            using (var db = new SQLiteConnection(DbConst.DbPath))
            {
                var result = from record in db.Table<SubPartModel>()
                    where record.ParentPartId == parentPartId
                    orderby record.SubPartName
                    select record;
                return result.Any() ? result.ToList() : new List<SubPartModel>();
            }
        }
        ...
        /// <param name="subPartName"></param>
        /// <returns></returns>
        public bool UpdateSubPart(int id, int parentPartId, string subPartName)
        {
            var model = new SubPartModel
            {
                Id = id,
